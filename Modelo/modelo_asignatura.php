...
class asignatura{
    ...
    private $descripcion_as;
    private $profesor_as;

    function __construct($descripcion_as,$profesor_as)
    {


        $this->descripcion_as = $descripcion_as;
        $this->profesor_as =  $profesor_as;
    ...
    function listarTablaAsistencia(){
        include('conexion.php');
        $id=null;
        $ver_usuario = $bd ->query("SELECT * FROM asignaturas_at ;")->fetchAll(PDO::FETCH_OBJ);
        return json_encode($ver_usuario,JSON_UNESCAPED_UNICODE);
        $bd=null;
    }
